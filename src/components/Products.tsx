...
function ProductCard({ product, index }: { product: any, index: number }) {
  ...
  return (
    <motion.div
      ...
      className={`bg-black/45 backdrop-blur rounded-[2rem] border ${product.highlight ? 'border-aura-lime/60 shadow-xl shadow-aura-lime/10' : 'border-white/10'} p-7 hover:border-aura-lime/60 transition-all group flex flex-col relative overflow-hidden`}
    >
      <div className="pointer-events-none absolute -right-16 -top-16 h-40 w-40 rounded-full bg-aura-gold/10 blur-3xl opacity-0 transition-opacity group-hover:opacity-100" />
      ...
      <div className="aspect-video rounded-2xl overflow-hidden bg-zinc-900 mb-8 items-center justify-center flex border border-white/5">
        <img 
          src={product.image} 
          alt={product.name} 
      ...
      <div className="flex items-center gap-3">
        <div className="flex items-center bg-zinc-950/80 border border-white/10 rounded-xl p-1 shrink-0">
          <button 
            onClick={decrement}
      ...
      <div className="mt-4 grid grid-cols-3 gap-2 text-center text-[8px] font-bold uppercase tracking-[0.16em] text-zinc-500">
        <span className="rounded-lg border border-white/10 py-2">Warranty</span>
        <span className="rounded-lg border border-white/10 py-2">Setup inc.</span>
        <span className="rounded-lg border border-white/10 py-2">No fees</span>
      </div>
    </motion.div>
  );
}
    ...
    <section id="products" className="py-24 border-t border-white/10">
      <div className="max-w-7xl mx-auto px-6">
        <div className="mb-16 text-center">
          <div className="text-aura-gold text-[10px] font-black uppercase tracking-[0.42em] mb-4">Solo Hardware</div>
          <h2 className="text-4xl md:text-6xl font-display font-black uppercase tracking-[-0.055em]">Standard <span className="text-aura-gold">Units.</span></h2>
